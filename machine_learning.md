...
Ordinary least squares regression, as described above, defines the bias/noise term as a random variable but the coefficients themselves are exact estimates. Bayesian Linear Regression also defines the coefficients as a random variables. Using Bayes Theorem, you can estimate the posterior distribution of the coefficients and bias: posterior(β, σ² | Y, X) ∝ likelihood(Y | X, β, σ²) * prior(β, σ²). Using domain knowledge, you can define priors for betas and variances, allowing you to better tune the model and improve accuracy. Defining the priors also tends to have a regularization effect and is less likely to overfit as OLS regression. Finally, the parameters predicted by the model have a probabilistic interpretation, which can be useful for cerrtain decision-making. The drawbacks of BLR is that it is computationally expensive since it requires MCMC methods to estimate.
...
For more details, try this [article](https://towardsdatascience.com/understanding-auc-roc-curve-68b2303cc9c5).
...
#### Anomaly detection
This is typically done by running one of the above clustering methods and labeling any outliers or very small clusters.
...
SOMs are a model that learn to project high-dimensional data into a lower-dimensional place, typically a 2D grid. This is done by assigning nodes, each with a weight vector the same dimensionality as the input, to points on a grid, typically a rectangular or hexagonal grid. Then, during training, the node whose weight vector is closest to an input sample is calculated by computing distances. This Best Matching Unit and a small neighborhood of nodes around it are updated to be closer to the input sample modified by learning rate. Over the course of training the model learns to capture clusters of input vectors that are similar to each other. As a result, you can visualize the high-dimensional ddata by projecting the input vector to its closest node on the grid. This can also be used for clustering and anomaly detection.

Compared to other dimensionality reduction methods such as t-SNE and UMAP, SOMs tend to capture global structure better and are more computationally efficient for larger datasets. They can also capture nonlinear structure, unlike PCA. However, SOMs have a few drawbacks: 1) they are sensitive to initialization, and different initial conditions can produce different maps. 2) they are sensitive to hyperparameters such as grid size and shape and there is no clear guidance on how to choose these

### Hidden Markov Models
HMMs are a statistical model that assume there are underlying hidden states governed by a Markov process (state machine with transitions only dependent on current state and not past states) and the data we observe is a function of those states. The parameters of the model are transition probabilities between states and emission probabilities (probability a state will generate an observation). These parameters are learned through a variant of Expectation Maximization.
...
## Adversarial ML
Adversarial ML is learning a proxy model to understand perturbations that "trick" another model. If the adversary is a classifier, we want to learn an inferred model to generate attacks that can cause it to misclassify. If the adversary is a regressor, we want to learn an inferred model that can generate attacks to cause it be wildly off the target value. For example, say you want to "hack" a self-driving car that uses a CNN for object detection on the road. The inferred model may be another CNN, and you use this inferred model to understand which inputs would cause the model to misclassify objects. Then, you use these inputs as attacks and transmit it to the car's model to cause it to mislabel. This may manifest in some intelligent noise added to the camera to mislabel a stop sign as a green light, for example.

To achieve this, we must first learn an inferred model. This can be a similar architecture to the adversary or just anything with sufficient complexity to handle the input-outputs and the same task as the adversary. Typically you would use neural networks to approximate any type of model. Now, you generate attacks in three phases:
1) _Pre-training_: If you can gather a dataset that emulates the adversary's training set you can train your inferred model to be already close to the adversary's performance, but this depends on availability of the data, how expensive it is to gather it, how close it can be to the actual training set, etc. In cases where this is not possible, you may want to _observe_ the adversary's input and outputs and gather a training set this way. Either way, you may have a limited amount of data to train with.
2) _Dataset augmentation_: To get similar performance as quickly as possible, we can use _Jacobian dataset augmentation_ to quickly probe the decision boundaries of the adversary. In JDA, we calculate the jacobian of the output of the adversary with respect to an input sample for the inferred model. Then, we perturb this sample by the sign of the Jacobian weighted by lambda. Now we have a new sample that is slightly closer to a decision boundary we can train on. The goal is to use this augmentation to quickly approximate the adversary with minimal data.
3) _Craft attacks_: We can now use the inferred model as a proxy for the adversary and find perturbations that maximize the inferred model's loss. There are several methods to achieve this, I will explain too. The _fast gradient sign method_ lets you perturb against a particular class label by computing the gradient of the label with respect to an input sample using the inferred model, then adding the sign of the gradient weighted by epsilon to an input sample to craft an attack. It's very similar to JDA, except you are attacking a particular label to be misclassified as anything else. _Jacobian saliency map attack_ creates attacks to misclassify from a specific label to another by calculating saliency values based on jacobian of target class w.r.t. input sample multiplied by magnitude of jacobians of every other class w.r.t. input. These saliency values are sorted and the top few are added to input until misclassification occurs. JSMA is more computationally expensive than FGSM, but it gives more control.

A great foundational paper describing these attacks is [Papernot et al. 2016](https://arxiv.org/abs/1602.02697).
