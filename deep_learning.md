...
### Batch normalization
Batch normalization is a differentiable layer in between the output of one hidden layer and the input to another that normalizes the mean and variance of a minibatch of samples for each feature. It does this by first normalizing to zero mean and unit variance, then apply two scaling parameters, gamma and beta, to modify the variance and mean, respectively. These are learnable parameters, so batchnorm has the ability to learn through training what mean and variance coming out from this layer are best for performance, and it may not necessarily be zero mean unit variance. For inference, we have to first normalize the input, but it is one sample and not a whole minibatch. How do we subtract mean and divide by variance then? Ideally you would keep the mean and variance of the entire training dataset but that would mean loading the entire dataset into memory in order to calculate that. So as an approximation, batch norm does this by keeping track of exponential moving average of mean and variance as it's training, so that it is incrementally updated as it sees minibatches and provides a close estimate to the actual mean and variance of the dataset. This is used during inference to normalize samples, then the sample is rescaled by gamma and beta. More info can be found in this [article](https://towardsdatascience.com/batch-norm-explained-visually-how-it-works-and-why-neural-networks-need-it-b18919692739).

Batchnorm directly addresses covariate shift, or when the distribution of activations changes from layer to layer. Networks converge faster by having layer activations close to the ideal mean and variance, activations functions tend to be used in their more useful regime. It helps promote gradient flow in this way. The impact of weight initialization is less drastic since BN can refocus badly initialized layers. It also adds a bit of noise between layers, acting as a form of regularization. It is generally advised to include batch normalization between every layer.

### Layer normalization
Layer normalization also attempts to address the covariate shift problem, but does this by computing statistics across neurons/channel/feature dimension instead of the batch dimension. This results in mean and variance for each data point, which means no different behavior for inference. It also makes it effective for tasks with variable or small batch sizes, like in NLP with sequence models and transformers. It treats each sequence independently instead of part of the same population as in batch norm. For these reasons, it is widely used in transformers. Layer norm also learns a gamma and beta parameter.s
